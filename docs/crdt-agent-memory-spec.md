...
- 検索は `FTS5 + vector` を基盤にした recall に trust/signature, graph, artifact, temporal の信号を重ねる
- ベクトルはローカル生成・ローカル再索引
...
- これは重み付けの例であり、実装では細部の係数が調整される
- 現行実装は semantic / lexical / trust / graph / artifact / recency を併用している
...
- `relations[]` は `memory_edges` へ保存される
- `scope='private'` の場合は `private_memory_edges` に保存される
- `supersedes` は `SupersedeMemory` 専用で、`StoreMemory` の `relations[]` では扱わない
...
3. `FTS5 + vector` の hybrid recall を作る
