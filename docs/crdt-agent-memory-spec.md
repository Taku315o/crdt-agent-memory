...
- 検索は `FTS5 + vector` を基盤にした recall と trust/signature weight
- graph/temporal boost は将来の rerank 拡張として扱う
- ベクトルはローカル生成・ローカル再索引
...
- これは設計目標の配分例であり、現行実装の recall そのものではない
- 現行実装は semantic candidate / lexical candidate に対して trust/signature bucketing を加えている
...
- FTS5 + vector + trust/signature-aware recall
- API 実装
...
3. `FTS5 + vector` の recall と trust/signature ordering を作る
