...
sequenceDiagram
    ...
    API->>DB: load graph connectivity and artifact span boosts
...
- current: artifact spans / quote hints
...
- graph / artifact 情報は `TraceDecision` でも追跡でき、`Recall` の rerank にも反映される
